...
# Macbook Pro M2 has no NVIDIA GPU thus does not support CUDA.jl, here choose 
# to set up an NVIDIA GPU instance(contains one or more GPUs) on AWS 
# Steps: 1. Launch an EC2 instance with NVIDIA GPUs
#        2. Set up SSH key pairs and security groups
#        3. Connect to the EC2 instance using SSH

# The following code may not run successfully since it has not been tested and 
# debugged, and all the operations, including scalar operations, are transferred 
# to be performed on GPUs (though this may not be necessary)

# (Since AWS charges monthly for running instances, I cannot start it until I am 
# sure that I will be approved to do this project)
# Sorry for that ʕ·ᴥ·ʔ
