...
bool areArraysEqual(float ***array1, double ***array2, int width, int height, int depth) {
    for (int z = 0; z < depth; ++z) {
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                if (std::abs(array1[z][y][x] - array2[z][y][x]) >
                    1e-6) { // using a small threshold for floating point comparison
                    return false;
                }
            }
        }
...
void test_copy_to_gpu() {
    ...
    for (int z = 0; z < depth; ++z) {
        ...
        for (int y = 0; y < height; ++y) {
            ...
            for (int x = 0; x < width; ++x) {
                ...
                u_host[z][y][x] =
                    static_cast<double>(x + y + z); // Some arbitrary initialization for testing
            }
        }
    }
    ...
    std::cout << "copy_to_gpu function was called. Verification functionality should be added."
              << std::endl;
