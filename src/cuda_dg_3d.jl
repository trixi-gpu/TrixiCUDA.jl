...
@generated function boundary_stable_helper(boundary_conditions,
                                           u_inner,
                                           orientation,
                                           direction,
                                           x,
    ...
    quote
        @nif $n d->d == direction d->return boundary_conditions[d](u_inner,
                                                                   orientation,
                                                                   direction,
                                                                   x,
                                                                   t,
...
function flux_kernel!(flux_arr1,
                      flux_arr2,
                      flux_arr3,
                      u,
...
function weak_form_kernel!(du, derivative_dhat, flux_arr1, flux_arr2, flux_arr3)
    ...
    if (i <= size(du, 1) && j <= size(du, 2)^3 && k <= size(du, 5))
        ...
        @inbounds begin
            for ii in axes(du, 2)
                du[i, j1, j2, j3, k] += derivative_dhat[j1, ii] *
                                        flux_arr1[i, ii, j2, j3, k]
                du[i, j1, j2, j3, k] += derivative_dhat[j2, ii] *
                                        flux_arr2[i, j1, ii, j3, k]
                du[i, j1, j2, j3, k] += derivative_dhat[j3, ii] *
                                        flux_arr3[i, j1, j2, ii, k]
            end
        end
    end
...
function volume_flux_kernel!(volume_flux_arr1,
                             volume_flux_arr2,
                             volume_flux_arr3,
                             u,
                             equations::AbstractEquations{3},
...
function symmetric_noncons_flux_kernel!(symmetric_flux_arr1,
                                        symmetric_flux_arr2,
                                        symmetric_flux_arr3,
                                        noncons_flux_arr1,
                                        noncons_flux_arr2,
...
function volume_integral_kernel!(du,
                                 derivative_split,
                                 volume_flux_arr1,
                                 volume_flux_arr2,
                                 volume_flux_arr3)
...
function volume_integral_kernel!(du,
    ...
    if (i <= size(du, 1) && j <= size(du, 2)^3 && k <= size(du, 5))
        ...
        @inbounds begin
            ...
            for ii in axes(du, 2)
                du[i, j1, j2, j3, k] += derivative_split[j1, ii] *
                                        symmetric_flux_arr1[i, j1, ii, j2, j3, k]
                du[i, j1, j2, j3, k] += derivative_split[j2, ii] *
...
function cuda_volume_integral!(du,
                               u,
                               mesh::TreeMesh{3},
    ...
    flux_kernel = @cuda launch=false flux_kernel!(flux_arr1,
                                                  flux_arr2,
                                                  flux_arr3,
                                                  u,
                                                  equations,
                                                  flux)
    flux_kernel(flux_arr1,
                flux_arr2,
                flux_arr3,
                u,
                equations;
    ...
    weak_form_kernel = @cuda launch=false weak_form_kernel!(du,
                                                            derivative_dhat,
                                                            flux_arr1,
                                                            flux_arr2,
                                                            flux_arr3)
...
function cuda_volume_integral!(du,
    ...
    volume_flux_arr1 = CuArray{Float32}(undef,
                                        size(u, 1),
                                        size(u, 2),
                                        size(u, 2),
                                        size(u, 2),
                                        size(u, 2),
                                        size(u, 5))
    volume_flux_arr2 = CuArray{Float32}(undef,
                                        size(u, 1),
                                        size(u, 2),
                                        size(u, 2),
                                        size(u, 2),
                                        size(u, 2),
                                        size(u, 5))
    volume_flux_arr3 = CuArray{Float32}(undef,
                                        size(u, 1),
                                        size(u, 2),
                                        size(u, 2),
                                        size(u, 2),
    ...
    volume_flux_kernel = @cuda launch=false volume_flux_kernel!(volume_flux_arr1,
                                                                volume_flux_arr2,
                                                                volume_flux_arr3,
                                                                u,
                                                                equations,
                                                                volume_flux)
    volume_flux_kernel(volume_flux_arr1,
                       volume_flux_arr2,
                       volume_flux_arr3,
                       u,
                       equations,
...
function cuda_volume_integral!(du,
    ...
    derivative_split = CuArray{Float32}(dg.basis.derivative_split)
    symmetric_flux_arr1 = CuArray{Float32}(undef,
                                           size(u, 1),
                                           size(u, 2),
                                           size(u, 2),
                                           size(u, 2),
                                           size(u, 2),
                                           size(u, 5))
    symmetric_flux_arr2 = CuArray{Float32}(undef,
                                           size(u, 1),
                                           size(u, 2),
                                           size(u, 2),
                                           size(u, 2),
                                           size(u, 2),
                                           size(u, 5))
    symmetric_flux_arr3 = CuArray{Float32}(undef,
                                           size(u, 1),
                                           size(u, 2),
                                           size(u, 2),
                                           size(u, 2),
                                           size(u, 2),
                                           size(u, 5))
    noncons_flux_arr1 = CuArray{Float32}(undef,
                                         size(u, 1),
                                         size(u, 2),
                                         size(u, 2),
                                         size(u, 2),
                                         size(u, 2),
                                         size(u, 5))
    noncons_flux_arr2 = CuArray{Float32}(undef,
                                         size(u, 1),
                                         size(u, 2),
                                         size(u, 2),
                                         size(u, 2),
                                         size(u, 2),
                                         size(u, 5))
    noncons_flux_arr3 = CuArray{Float32}(undef,
                                         size(u, 1),
                                         size(u, 2),
                                         size(u, 2),
                                         size(u, 2),
    ...
    symmetric_noncons_flux_kernel(symmetric_flux_arr1,
                                  symmetric_flux_arr2,
                                  symmetric_flux_arr3,
                                  noncons_flux_arr1,
                                  noncons_flux_arr2,
                                  ...
                                  symmetric_flux,
                                  nonconservative_flux;
                                  configurator_2d(symmetric_noncons_flux_kernel,
                                                  size_arr)...,)
    ...
    if (j <= size(interfaces_u, 2) * size(interfaces_u, 3)^2 && k <= size(interfaces_u, 5))
        ...
        @inbounds begin
            interfaces_u[1, j1, j2, j3, k] = u[j1,
                                               isequal(orientation, 1) * size(u, 2) + isequal(orientation, 2) * j2 + isequal(orientation,
                                               3) * j2,
                                               isequal(orientation, 1) * j2 + isequal(orientation, 2) * size(u,
                                                                                                             2) + isequal(orientation,
                                               3) * j3,
                                               isequal(orientation, 1) * j3 + isequal(orientation, 2) * j3 + isequal(orientation,
                                               3) * size(u, 2),
                                               left_element]
            interfaces_u[2, j1, j2, j3, k] = u[j1,
                                               isequal(orientation, 1) * 1 + isequal(orientation, 2) * j2 + isequal(orientation,
                                               3) * j2,
                                               isequal(orientation, 1) * j2 + isequal(orientation, 2) * 1 + isequal(orientation,
                                               3) * j3,
                                               isequal(orientation, 1) * j3 + isequal(orientation, 2) * j3 + isequal(orientation,
                                               3) * 1,
                                               right_element]
...
function cuda_prolong2interfaces!(u, mesh::TreeMesh{3}, cache)
    ...
    prolong_interfaces_kernel = @cuda launch=false prolong_interfaces_kernel!(interfaces_u,
                                                                              ...
                                                                              neighbor_ids,
                                                                              orientations)
    prolong_interfaces_kernel(interfaces_u,
                              u,
                              neighbor_ids,
                              orientations;
...
function surface_flux_kernel!(surface_flux_arr,
                              interfaces_u,
                              orientations,
                              equations::AbstractEquations{3},
                              surface_flux::Any)
...
function surface_noncons_flux_kernel!(surface_flux_arr,
                                      interfaces_u,
                                      noncons_left_arr,
                                      noncons_right_arr,
                                      orientations,
...
function interface_flux_kernel!(surface_flux_values,
                                surface_flux_arr,
                                neighbor_ids,
                                orientations)
    i = (blockIdx().x - 1) * blockDim().x + threadIdx().x
    ...
    if (i <= size(surface_flux_values, 1) &&
        ...
        @inbounds begin
            surface_flux_values[i, j1, j2, left_direction, left_id] = surface_flux_arr[1, i,
                                                                                       j1,
                                                                                       j2,
                                                                                       k]
            surface_flux_values[i, j1, j2, right_direction, right_id] = surface_flux_arr[1,
                                                                                         i,
                                                                                         j1,
                                                                                         j2,
                                                                                         k]
...
function cuda_interface_flux!(mesh::TreeMesh{3},
                              nonconservative_terms::False,
                              equations,
                              dg::DGSEM,
                              cache)
    ...
    surface_flux_kernel = @cuda launch=false surface_flux_kernel!(surface_flux_arr,
                                                                  ...
                                                                  equations,
                                                                  surface_flux)
    surface_flux_kernel(surface_flux_arr,
                        interfaces_u,
                        orientations,
                        equations,
                        surface_flux;
    ...
    interface_flux_kernel = @cuda launch=false interface_flux_kernel!(surface_flux_values,
                                                                      ...
                                                                      neighbor_ids,
                                                                      orientations)
    interface_flux_kernel(surface_flux_values,
                          surface_flux_arr,
                          neighbor_ids,
                          orientations;
...
function cuda_interface_flux!(mesh::TreeMesh{3},
    ...
    surface_noncons_flux_kernel = @cuda launch=false surface_noncons_flux_kernel!(surface_flux_arr,
                                                                                  ...
                                                                                  surface_flux,
                                                                                  nonconservative_flux)
    surface_noncons_flux_kernel(surface_flux_arr,
                                interfaces_u,
                                noncons_left_arr,
                                noncons_right_arr,
                                orientations,
...
function prolong_boundaries_kernel!(boundaries_u,
                                    u,
                                    neighbor_ids,
                                    neighbor_sides,
                                    orientations)
...
# Assert 
function cuda_prolong2boundaries!(u,
                                  mesh::TreeMesh{3},
...
function cuda_prolong2boundaries!(u,
    ...
    prolong_boundaries_kernel = @cuda launch=false prolong_boundaries_kernel!(boundaries_u,
                                                                              u,
                                                                              neighbor_ids,
                                                                              neighbor_sides,
                                                                              orientations)
...
function boundary_flux_kernel!(surface_flux_values,
                               boundaries_u,
                               node_coordinates,
                               t,
                               boundary_arr,
    ...
    if (j <= size(surface_flux_values, 2)^2 && k <= length(boundary_arr))
        ...
        direction = (indices_arr[1] <= boundary) +
...
function cuda_boundary_flux!(t,
    ...
    last_first_indices_kernel = @cuda launch=false last_first_indices_kernel!(lasts,
                                                                              firsts,
                                                                              n_boundaries_per_direction)
    last_first_indices_kernel(lasts,
                              firsts,
                              n_boundaries_per_direction;
    ...
    boundary_flux_kernel = @cuda launch=false boundary_flux_kernel!(surface_flux_values,
                                                                    ...
                                                                    equations,
                                                                    surface_flux)
    boundary_flux_kernel(surface_flux_values,
                         boundaries_u,
                         node_coordinates,
                         t,
                         boundary_arr,
...
function prolong_mortars_small2small_kernel!(u_upper_left,
                                             u_upper_right,
                                             u_lower_left,
                                             u_lower_right,
                                             u,
    ...
    if (i <= size(u_upper_left, 2) &&
        ...
        @inbounds begin
            u_upper_left[2, i, j1, j2, k] = u[i,
                                              isequal(orientation, 1) * 1 + isequal(orientation, 2) * j1 + isequal(orientation,
                                              3) * j1,
                                              isequal(orientation, 1) * j1 + isequal(orientation, 2) * 1 + isequal(orientation,
                                              3) * j2,
            ...
            u_upper_left[1, i, j1, j2, k] = u[i,
                                              isequal(orientation, 1) * size(u, 2) + isequal(orientation, 2) * j1 + isequal(orientation,
                                              3) * j1,
                                              isequal(orientation, 1) * j1 + isequal(orientation, 2) * size(u,
                                                                                                            2) + isequal(orientation,
                                              3) * j2,
                                              isequal(orientation, 1) * j2 + isequal(orientation, 2) * j2 + isequal(orientation,
                                              3) * size(u, 2),
...
function prolong_mortars_large2small_kernel!(u_upper,
                                             u_lower,
                                             u,
                                             forward_upper,
                                             forward_lower,
    ...
    if (i <= size(du, 1) && j <= size(du, 2)^3 && k <= size(du, 5))
        ...
        @inbounds begin
            ...
            du[i, j1, j2, j3, k] += (surface_flux_values[i, j2, j3, 2, k] *
                                     ...
                                     surface_flux_values[i, j1, j2, 6, k] *
                                     isequal(j3, size(du, 2))) * factor_arr[2]
        end
    end
...
function source_terms_kernel!(du,
                              u,
                              node_coordinates,
                              t,
                              equations::AbstractEquations{3},
...
function cuda_sources!(du, u, t, source_terms, equations::AbstractEquations{3}, cache)
    ...
    source_terms_kernel = @cuda launch=false source_terms_kernel!(du,
                                                                  u,
                                                                  node_coordinates,
                                                                  t,
                                                                  equations,
                                                                  source_terms)
    source_terms_kernel(du,
                        u,
                        node_coordinates,
                        t,
                        equations,
