    ...
    if (i <= size(du, 1) && j <= size(du, 2)^3 && k <= size(du, 5))
        ...
        @inbounds begin
            if element_dg != 0 # bad
                ...
                du[i, j1, j2, j3, element_dg] += 0.5f0 * integral_contribution
            end
            ...
            if element_dgfv != 0 # bad
                ...
                du[i, j1, j2, j3, element_dgfv] += 0.5f0 * (1 - alpha_element) * integral_contribution
            end
        end
    end
...
function cuda_mortar_flux!(mesh::TreeMesh{3}, cache_mortars::True, nonconservative_terms::False,
    ...
    # TODO: Combine these two copy kernels into one (synchronization)
